...
import com.wuying.browser.data.BookmarkManager
...
class BookmarksActivity : AppCompatActivity() {
    ...
    private val adapter = BookmarksAdapter { url ->
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        rv = findViewById(R.id.rv_bookmarks)
        rv.layoutManager = LinearLayoutManager(this)
        rv.adapter = adapter
        lifecycleScope.launch {
            adapter.submit(BookmarkManager.get(this@BookmarksActivity).all())
        }
    }

    override fun onSupportNavigateUp(): Boolean { finish(); return true }

    private class BookmarksAdapter(val onClick: (String) -> Unit) : RecyclerView.Adapter<BookmarksAdapter.VH>() {

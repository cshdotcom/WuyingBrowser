...
当前版本默认唤出方式：
- 桌面图标（未隐藏时）→ 主浏览器界面
- 后台通知点击（「系统更新」伪装常驻通知，`CoreService` / `DaemonService` 前台通知）→ **直达悬浮窗浏览器面板**（v1.3.0 起）
- 悬浮球点击 → 恢复悬浮窗浏览器面板
...
### 悬浮窗浏览器（v1.3.1）
点击通知即出现一块可拖动的浏览器小面板，浮在当前应用之上：
- **面板**：可拖动（按住标题栏）、两档大小切换、**右下角把手任意缩放**（v1.3.1 新增，拖动范围 220dp×170dp 至接近全屏）、最小化为悬浮球、点击面板外空白最小化
- **输入法**：面板由全屏透明 Activity 承载（标准应用窗口），网页输入框 / 地址栏键盘唤起正常 —— v1.1/v1.2 的 overlay 窗口在大量 ROM 上无法唤起输入法，v1.3.0 已根治
- **悬浮球**：overlay 常驻小球，可拖动贴边，单击恢复面板；按 Home 离开时面板自动收成悬浮球，体验与真悬浮窗一致
- **数据互通**：与主浏览器共用同一数据目录，Cookie / 缓存 / 历史 / 会话完全同步
